...
first_anomalies_df = pd.DataFrame({"timestamp": first_random_date_df,"speed": low_speed_df["speed"],"rpm":high_rpm_df["rpm"]})
second_anomalies_df = pd.DataFrame({"timestamp": second_random_date_df,"speed": high_speed_df["speed"],"rpm":low_rpm_df["rpm"]})
...
complete_dataframe = joined_df.drop("timestamp",axis=1)
